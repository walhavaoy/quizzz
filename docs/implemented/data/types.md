...
| REQ-TY-03   | done   | `GameState` interface with `players: Player[]` and `hostId` in `src/shared/types.ts` |
| REQ-TY-04   | done   | `ServerMessage` and `ClientMessage` discriminated unions in `src/shared/types.ts` |
| REQ-TY-05   | done   | `CreateGameResponse`, `JoinRequest/Response`, `AnswerRequest/Response`, `ErrorResponse`, `SubmitAnswerRequest/Response`, `ApiErrorResponse` in `src/shared/types.ts` |
...
- `GameState.players` uses `Player[]` for server-side use. Wire messages use the same array shape for JSON serialization.
